    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        let (result, output_language, model_languages) = {
            ...
            if let LoadedEngine::TranscribeCpp(session) = &engine {
                ...
                // Feature::InitialPrompt. Informational (logged below); the
                // whisper run extension and the fuzzy-correction skip are
                // gated on `model_is_whisper` instead, since non-whisper
                // archs can advertise the feature while rejecting the
